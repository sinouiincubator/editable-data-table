...
我们可以通过给`EditableDataTable`组件一个 `rowClassName`属性来动态指定一个样式名称，从而达到根据状态定制数据行状态的目的。
...
当数据行被选中时，我们给其指定一个`sinoui-data-table-body-row_selected`样式类名称，然后在 css 文件中定义选其样式。
