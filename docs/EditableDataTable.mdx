...
<Playground>
  {() => {
    ...
    const [editingRows, setEditingRows] = useState([1]);
    ...
    const handleSave = (row, index, context) => {
      if (!context.validate()) {
        ...
        setEditingRows(prev => prev.filter(item => item !== index));
        alert('保存成功');
      }
    };

    const handleEdit = (index) => {
      setEditingRows(prev => [...prev, index]);
    };
    ...
    const handleAdd = () => {
      setEditingRows([...editingRows, items.length]);
      setItems([...items, {}]);
    };
